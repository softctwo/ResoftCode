...
export const UninstallCommand = {
  command: "uninstall",
  describe: `uninstall ${Brand.product} and remove all related files`, // kilocode_change
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro(`Uninstall ${Brand.product}`) // kilocode_change
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success(`Thank you for using ${Brand.product}!`) // kilocode_change
}

...
const label = Brand.product.toUpperCase()

const fallback = {
  tui: [label, `------------------`, Brand.tagline, ``],
  plain: [label, `------------------`, Brand.tagline],
  exit: [`  ${label}`, `  ------------------`, `  ${Brand.tagline}`],
}
...
export function session(
  ...
  return [
    ...
    `${logo[1]}${dim}${Brand.product} session ${id}${normal}`,
    logo[2],
  ].join("\n")
}
